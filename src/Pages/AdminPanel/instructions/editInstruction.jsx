...
function EditInstruction({
  ...
  const textParse = useCallback((str) => (
    str.split(' | ').map((a, i) => ({ ingredient: a, index: i }))
  ), []);
  ...
  const initialState = useMemo(() => ({
    ...
    ingredients: textParse(item.ingredient),
  ...
  const handleSubmitForm = (values) => {
    ...
    values.text = values.text
      .map((step) => step.ingredient)
      .join(' | ');

    const request = removeUnchangedFields(item, values);

    editDataConfig('instruction', item.id, axiosPrivateConfig, request)
      .then(() => {
              ...
              <Form.Group className="form-element">
                ...
                <Form.Group className="control-element">
                  <FieldArray className="rendered-content" name="ingredients">
                    {() => values.ingredients.map((itemIng, i) => (
                      <Form.Group
                        key={`ingredients${i}`}
